...
use QExcel\RichText;
use QExcel\Shared\CodePage;
use QExcel\Shared\ExcelDate;
use QExcel\Shared\ExcelString;
use QExcel\Shared\OLERead;
use QExcel\Workbook;
use QExcel\Worksheet;
 ...
 * @author      PHPExcel
 */
class Excel5 extends AbstractReader
{
	// ParseXL definitions
...
//		$recordData = substr($this->_data, $this->_pos + 4, $length);
	...
	private function _readCodepage()
	{
		...
		$this->_codepage = CodePage::NumberToName($codepage);
	}
	...
	private function _readRk()
	{
        ...
        $this->_phpSheet->setCell(
            $row,
            $column,
            $this->getCellValue($numValue, DataType::TYPE_NUMERIC)
        );
	}
    ...
    private function _readLabelSst()
    {
        ...
        $this->_phpSheet->setCell(
            $row,
            $column,
            $this->getCellValue($this->_sst[$index]['value'], DataType::TYPE_STRING)
        );
    }
	...
	private function _readFormula()
	{
        ...
        if (!$isPartOfSharedFormula) {
            ...
            try {
                ...
                $this->_phpSheet->setCell($row, $column, $this->getCellValue('=' . $formula, DataType::TYPE_FORMULA));
	...
	private function _readBoolErr()
	{
        ...
        switch ($isError) {
            case 0: // boolean
                $value = (bool) $boolErr;
                $this->_phpSheet->setCell($row, $column, $this->getCellValue($value, DataType::TYPE_BOOL));
                break;

            case 1: // error type
                $value = self::_mapErrorCode($boolErr);
                $this->_phpSheet->setCell($row, $column, $this->getCellValue($value, DataType::TYPE_ERROR));
                break;
        }
	}
	...
	private function _readLabel()
	{
        ...
        $this->_phpSheet->setCell($row, $column, $this->getCellValue($value, DataType::TYPE_STRING));
	}
	...
	private static function _encodeUTF16($string, $compressed = '')
	{
		...
		return ExcelString::ConvertEncoding($string, 'UTF-8', 'UTF-16LE');
	}
